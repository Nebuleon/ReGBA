...
#include "regs.h"		/* jz4730 definitions */
#include "ops.h"		/* jz4730 definitions */
...
static int jz_mmc_receive_data_dma(struct mmc_request *req)
{
	...
	REG_DMAC_DDAR(ch) = PHYSADDR((unsigned long) req->buffer);	/* DMA dest addr */
	REG_DMAC_DTCR(ch) = (size + 31) / 32;	/* DMA transfer count */



	/* wait for dma completion */
#ifdef MMC_DMA_INTERRUPT
	//printf("receive interrup!\r\n");
	REG_DMAC_DCCSR(ch) = DMAC_DCCSR_DAM | DMAC_DCCSR_SWDH_32 | DMAC_DCCSR_DWDH_32 | DMAC_DCCSR_DS_32B | DMAC_DCCSR_CHDE | DMAC_DCCSR_TCIE;	/* DMA channel control/status register */
	OSSemPend(mmc_dma_sem, 100, &err);
#else
	REG_DMAC_DCCSR(ch) = DMAC_DCCSR_DAM | DMAC_DCCSR_SWDH_32 | DMAC_DCCSR_DWDH_32 | DMAC_DCCSR_DS_32B | DMAC_DCCSR_CHDE;	/* DMA channel control/status register */
	while (!(REG_DMAC_DCCSR(ch) & DMAC_DCCSR_TC));
#endif
...
static int jz_mmc_transmit_data_dma(struct mmc_request *req)
{
	...
	REG_DMAC_DDAR(ch) = PHYSADDR(MSC_TXFIFO);	/* DMA dest addr */
	REG_DMAC_DTCR(ch) = (size + 31) / 32;	/* DMA transfer count */
	...
	REG_DMAC_DCCSR(ch) = DMAC_DCCSR_SAM | DMAC_DCCSR_SWDH_32 | DMAC_DCCSR_DWDH_32 | DMAC_DCCSR_DS_32B | DMAC_DCCSR_CHDE | DMAC_DCCSR_TCIE;	/* DMA channel control/status register */

	OSSemPend(mmc_dma_sem, 100, &err);
#else
	REG_DMAC_DCCSR(ch) = DMAC_DCCSR_SAM | DMAC_DCCSR_SWDH_32 | DMAC_DCCSR_DWDH_32 | DMAC_DCCSR_DS_32B | DMAC_DCCSR_CHDE;	/* DMA channel control/status register */
	while (!(REG_DMAC_DCCSR(ch) & DMAC_DCCSR_TC));
...
#if defined(MMC_DMA_ENABLE)
			if (!jz_mmc_receive_data_dma(request))
				return MMC_ERROR_TIMEOUT;
#else
...
#if defined(MMC_DMA_ENABLE)
			if (!jz_mmc_transmit_data_dma(request))
				return MMC_ERROR_TIMEOUT;
#else
...
#ifdef MMC_DMA_INTERRUPT
void jz_mmc_handler(unsigned int arg)
{
...
#ifdef MMC_DMA_INTERRUPT
	mmc_dma_sem = OSSemCreate(0);
	request_irq(IRQ_DMA_0 + DMA_CHANNEL, jz_mmc_handler, DMA_CHANNEL);

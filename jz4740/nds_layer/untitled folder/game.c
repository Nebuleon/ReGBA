...
#define FRAME_BUFF_ADDRESS1 123

 struct MP4_dev* MP4_devices=NULL;
 unsigned char MP4_inc=0;
...
struct rtc * pnds_sys_rtc = NULL;
 ...
 struct nds_iqe_list_st nds_iqe_list[0x10];
 int nds_iqe_list_w;
 int nds_iqe_list_r;
...
#ifdef HTW_MP4_NDS
 // __DECLARE_SEMAPHORE_GENERIC(MP4_keybd_cnt, 0);
 //extern unsigned long volatile __jiffy_data jiffies;
unsigned int nds_updat_tim = 0;
 
#endif
 int MP4_open(struct inode *inode, struct file *filp)
{
#if 0
	struct MP4_dev *dev;
#ifndef HTW_MP4_NDS
 ...
 int MP4_release(struct inode *inode, struct file *filp)
{
#if 0
	MP4_inc--;
	return 0;
 ...
 ssize_t MP4_write(struct file *filp, const char __user *buf, size_t count,loff_t *f_pos)
{
    ...
    /*
 irqreturn_t jz4740_dma_irq1(int irq, void *dev_id)
{
 ...
 int dma_nodesc_write(u32 s,int len,int enable_iqe)
{
    int units = len /32 ;
...
#endif
	REG_DMAC_DTAR(dma_chan) = cpld_fifo_addr & 0x1fffffff;
	REG_DMAC_DTCR(dma_chan) = units;//len / 32;
    //if (enable_iqe ==1)
    //{
    ...
    //else
    //{
        REG_DMAC_DCMD(dma_chan) = DMAC_DCMD_SAI | DMAC_DCMD_SWDH_32 | DMAC_DCMD_DWDH_32 | DMAC_DCMD_DS_32BYTE;//DMAC_DCMD_DS_32BYTE ;//| DMAC_DCMD_TIE;
    //}
 ...
 void Enable_game_data_interrup(void)
{
...
__gpio_as_disenable_irq(fpga_cmd_port + nds_cmd_iqe_pinx);
 ...
 irqreturn_t cmd_line_interrupt(int irq, void *dev_id)
{
    u8 nds_iqe_cmd;
    int i;
    u32 tempw,tempr;
    ...
    cmd_buf32[0]=*(vu32*)cpld_fifo_addr;
    cmd_buf32[1]=*(vu32*)cpld_fifo_addr;
    nds_iqe_cmd=cmd_buf[0];
    isc1cmd=0;
    switch ( nds_iqe_cmd )
    {
        case 0xc0://返回cpu 请求中断
            *(vu8*)cpld_ctr_addr = (1<<fpga_mode_bit) |  (1<<fifo_clear_bit);
            *(vu8*)cpld_ctr_addr = (1<<fpga_mode_bit);
            *(vu8*)cpld_state_addr = 0;
            i = nds_iqe_list_w - nds_iqe_list_r;
            if (i != 0 && nds_iqe_list_w > nds_iqe_list_r)
            {
           
                dma_nodesc_write(nds_iqe_list[nds_iqe_list_r&0xf].c0_addr ,512,0);
        ...
        case 0xc1://NDS读 512*n
            isc1cmd = 1 << enable_fix_video_bit;
        case 0xc2://NDS读 512*n
            REG8(write_addr_cmp_addr) = (0x400-0x200)/4 ;
            *(vu8*)cpld_ctr_addr = (1<<fpga_mode_bit) |  (1<<fifo_clear_bit) | isc1cmd;
            *(vu8*)cpld_ctr_addr = (1<<fpga_mode_bit) | isc1cmd;
            ...
            else
            {
               ...
               dma_nodesc_write(nds_iqe_addr ,fpga_fifo_size,0);
...
//printf("keyp %08x\n", keyp->key);
        ...
        case 0xc4://NDS iqe
    		...
    		*(vu32*)cpld_fifo_addr = tempw | 0xa5a50000; 
            ...
            {
                if (pmain_buf->nds_audio_w < tempw)//when error
                {
...
//                nds_updat_tim = os_TimeGet();
		    ...
		    dma_cache_wback((unsigned long)(nds_iqe_st_p), (unsigned long)512);
			if(nds_iqe_st_p->iqe_cmd == 0xc2)//==0xc0 出错
			{
		        	dma_cache_wback((unsigned long)(nds_iqe_st_p->iqe_cpuaddr + kmalloc_ptr), (unsigned long)nds_iqe_st_p->iqe_len);
			}
	    }	
	    }
#endif 
 ...
 irqreturn_t data_line_interrupt(int irq, void *dev_id)
{
...
//    u32 tempw,tempr;
    struct nds_iqe_st *nds_iqe_st_p;
	  ...
	  if(cmd==SET_CTR)
	  {
            *(vu8*)cpld_ctr_addr = *(vu8*)cpld_ctr_addr | arg;
           return 0;
	  }
      else if (cmd==CLR_CTR)
      {
      ...
      else if (cmd==IQE_UPDATE)
      {
                ...
                if ((temp32 == 0) || ((temp32 & 0xf) !=0 ))         //command request queue is empty
                {
                    if (((u32)arg) >= buf_max_num)                  //command queue number error
                    {
                    ...
                    if (pmain_buf->buf_st_list[arg].isused != 0)
                    {
                        ...
                        if(arg == buf_video_up_0 || arg == buf_video_down_0){
                            if(pmain_buf->buf_st_list[arg].isused >= 2){
                                ...
                                // __LINE__,pmain_buf->buf_st_list[arg].isused);
                                //temp32=*(vu8*)cpld_state_addr;
                                //temp32 |= BIT(nds_iqe_out_bit);
                                //*(vu8*)cpld_ctr_addr = temp32;
                                *(vu8*)cpld_state_addr = BIT(nds_iqe_out_bit);
                ...
                if(((nds_iqe_st_p->iqe_cmd&0xff) == 0xc2)|| ((nds_iqe_st_p->iqe_cmd&0xff) == 0xc1)) 
                {
                    ...
                    switch ((nds_iqe_st_p->iqe_cmd>>8) & 0xff)
                    {
                        ...
                        case VIDEO_DOWN:
                            ...
                            break;
                        case AUDIO_G:                    
#ifdef AUDIO_PRI_FISRT
                            __gpio_mask_irq(fpga_cmd_port + nds_cmd_iqe_pinx);
                            {
                            int i;
                            for(i=nds_iqe_list_r; i < nds_iqe_list_w; i++){
                                if(nds_iqe_list[nds_iqe_list_r&0xf].num < buf_audio_0){
                                    struct nds_iqe_list_st tmp = nds_iqe_list[nds_iqe_list_r&0xf];
                                    nds_iqe_list[nds_iqe_list_r&0xf] = nds_iqe_list[nds_iqe_list_w&0xf];
			...
			if(((nds_iqe_st_p->iqe_cmd&0xff) == 0xc2)|| ((nds_iqe_st_p->iqe_cmd&0xff) == 0xc1)) 
			{
                dma_cache_wback((unsigned long)(nds_iqe_st_p->iqe_cpuaddr + kmalloc_ptr), 
...
#define __MP4_ioctl_dsp -1
 ...
 int __MP4_ioctl_fb(struct inode *inode, struct file *filp,unsigned int cmd, unsigned long arg)
{
    //int ret = -1;
...
struct fb_fix_screeninfo {
	...
	// struct fb_con2fbmap con2fb ={0,0};
	// struct fb_cmap_user cmap = {0,0,0,0,0,0};
 ...
 int __MP4_ioctl_dsp(struct inode *inode, struct file *filp,unsigned int cmd, unsigned long arg)
{
    int ret = -1;
...
#define  MP4_KEYBD_WAIT 0x7755
 int __MP4_ioctl_keybd(struct inode *inode, struct file *filp,unsigned int cmd, unsigned long arg)
{
...
#if 0
 int MP4_mmap(struct file *filp, struct vm_area_struct *vma)
{
 ...
 struct file_operations MP4_fops = {
	.owner	=	THIS_MODULE,
	.llseek	=	MP4_llseek,
	.read		=	MP4_read,
	.write	=	MP4_write,
...
REG_EMC_SMCR3 = ((3<<24) | //strv
    ...
    REG_EMC_SACR3 =( (0xc<<8)|//BASE
                    (0xfc<<0));//MASK
*/
REG_EMC_SMCR4 = ((3<<24) | //strv
                    (3<<20) | //taw r
                    (3<<16) | //tbp w
                    (3<<12) | //tah
                    (3<<8) | //tas
    ...
    REG_EMC_SACR4 =( (0x8<<8)|//BASE
                    (0xfc<<0));//MASK


	udelay(1);
}
 int init_nds_var(void)
{
    u32 *temp32p;
    int offset;
    //int i;
...
*/
 ...
 int my_init_fpga(void)
{

    *(vu8*)cpld_ctr_addr = BIT(fifo_clear_bit);
    udelay(200);
    *(vu8*)cpld_ctr_addr = 0;
    init_nds_var();
	return 0;
...
//#if ((FRAME_BUFF_ADDRESS1 + LEN - 1)&0xfff00000) > (FRAME_BUFF_ADDRESS1 &0xfff00000) 
...
pnds_sys_rtc = &pmain_buf->nds_rtc;
        ...
        buf_st_temp.nds_cmd=(buf_video_up_0<<16) | (VIDEO_UP <<8) | 0xc1;
        buf_st_temp.type= 0;
        pmain_buf->buf_st_list[buf_video_up_0] = buf_st_temp;

        buf_st_temp.offset=buf_st_temp.offset + buf_st_temp.len;
        buf_st_temp.nds_cmd= (buf_video_up_1<<16) | (VIDEO_UP <<8) | 0xc1;
        pmain_buf->buf_st_list[buf_video_up_1] = buf_st_temp;

        buf_st_temp.offset=buf_st_temp.offset + buf_st_temp.len;
        buf_st_temp.nds_cmd= (buf_video_down_0<<16) | (VIDEO_DOWN <<8) | 0xc1;
        pmain_buf->buf_st_list[buf_video_down_0] = buf_st_temp;

        buf_st_temp.offset=buf_st_temp.offset + buf_st_temp.len;
        buf_st_temp.nds_cmd= (buf_video_down_1<<16) | (VIDEO_DOWN <<8) | 0xc1;
    ...
    my_init_fpga();
#endif
	//dma_cache_wback((unsigned long)(kmalloc_ptr), (unsigned long)1024);

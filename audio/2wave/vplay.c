...
//	write (fd, &wh, sizeof(WaveHeader));
  	if (FS_FWrite (&wh,1,sizeof(WaveHeader),fd) != sizeof(WaveHeader)) {
...
void get_wav_set(void)
{
	dsp_speed  = codec_get_rate();
	dsp_stereo = (((int) codec_get_channels())==1)?0:1;
	samplesize = (codec_get_format()==AFMT_U8)?8:16;
//	printf("speed:%d stereo:%d samplesize:%d size wav:%d\n",dsp_speed,dsp_stereo,samplesize,sizeof(WaveHeader));
	
}

unsigned char recram[1024 * 1024];
...
//	 	 printf("read daata--%x-%x-%x-%x!\n",recram[recCount-4],recram[recCount-3],recram[recCount-2],recram[recCount-1]);
		 ...
		 l = pcm_read((char *)recram, 1024 * 128);
		 recCount = l;
	 ...
	 wavfile = FS_FOpen("wavenew.wav","wb");
	 get_wav_set();
	 start_wave(wavfile,recCount);
	 
	 if (FS_FWrite (recram,1,recCount,wavfile) != recCount) {
		 printf("can not write to File %s!!",fd);
